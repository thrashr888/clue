...
Keep stdout as versioned JSON except help/version and intentional skill-text export. Search is local by default. External ranking requires `--share-content`; explicit loopback System One/Ollama inference does not. Keep TypeSafe credentials isolated from local providers; use `CLUE_PROVIDER_API_KEY` for an explicitly configured local server. Never fall back from local inference to the cloud. Ollama ratings must not fabricate native probabilities or confidence. Shared credentials do not authorize sharing personal data. Real personal-name sharing was previously blocked by automatic approval review and has not been approved. Use synthetic content for API tests unless the user explicitly authorizes real content.
