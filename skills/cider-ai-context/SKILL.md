...
For meeting preparation, include `--sources calendar,reminders,notes` with explicit `--days-back` and `--days-ahead` bounds. Calendar retains structured start/end/all-day metadata; date words are not parsed into filters. Use the separate `cider-ai-sqlite` workflow for a database the user selects.
