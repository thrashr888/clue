...
| [GLiFormer](https://huggingface.co/knowledgator/gliformer-large-v1) / [Jeff](https://github.com/logan-markewich/jeff) | GLiFormer is a 575.6M encoder for classification/extraction. Jeff exposes a System One API around it. Extraction may be as useful as reranking for future Clue workflows. | Generic System One provider. Jeff documentation requires a server bearer key. No live Jeff validation in this change. |
...
Follow [Jeff's setup](https://github.com/logan-markewich/jeff), select a server key, and supply that same key through `CLUE_PROVIDER_API_KEY` (never command-line arguments). Use:
